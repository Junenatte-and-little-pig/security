...
graph LR
    ...
    w3-->sub32("如何防止篡改");
```
基于HTTP的无法改变的缺点(HTTP本身的便利性使得它不去做过多的检查，而是专注于保证传输本身的有效)，为了保证客户端及服务器传输过程的安全性，需要在HTTP基础上加上加密处理、认证以及完整性保护，于是就诞生了HTTPS。  
HTTPS并非一种新的协议，而是加上SSL(Secure Socket Layer)和TLS(Transport Layer Security)的HTTP，即在与TCP通信之前先经过SSL，用以保证安全。
### 加密
