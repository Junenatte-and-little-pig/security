...
### 质量保障
&emsp;&emsp;在IP协议中，由于对发送的数据包不加以控制，所有的数据包，包括一些质量不好的包，都可以直接在通信中进行传输，这就会导致网络拥挤的情况发生。因此，要使得网络能够在更好的情况下被使用，需要对包进行质量的筛选，确保高质量的包的传输。  
&emsp;&emsp;RSVP(Resource reservation Protocol 质量保留协议)用于从接收包的一端到发送端进行包的传输控制，在它们之间所包括的路由器中设定质量保证。  
&emsp;&emsp;为了实现质量控制的高速处理，人们采用MPLS(Multiprotocol Label Switching 多协议标号交换)协议，使用标号交换技术，在发送的时候给IP包设定标号，用来进行转发。
### IPv6
&emsp;&emsp;一方面，IPv4的地址数较少，已经不能够很好地适用于当今社会；另一方面，IPv4始终存在着自身不可避免的弊端，虽然在一定程度上可以通过增加限制来进行加强，但是这反而会降低它的便利性。因此，IPv6带着自己的使命出现在世人面前。IPv6在IPv4的基础上，提供了一些必要的功能，在加强功能性的同时，也保证了便利性。
- IP地址的扩大和路由控制表的聚集
- 提高了性能
- 必须具有标志和播放功能
- 采用了认证和加密功能
### ICMPv6
&emsp;&emsp;在IPv6中，ICMP变成必不可少的一部分，这给IPv6提高了安全性。另外，ICMPv6采用近邻检索消息来从IP地址查找MAC地址，具有重新导向、路由器选择信息、自动设定IP地址等功能。  
&emsp;&emsp;在ICMPv6中，返回的信息分成了两大类：错误信息和情报信息。
#### IPv6报头
- 版本
- 传输分类
- 流标号
...
- 扩展报头
## TCP协议和UDP协议
&emsp;&emsp;在传输层中，需要确定发送过来的包应该传输到哪一个应用程序，因此，定义了一个端口号，用来识别不同程序的不同通信，确保传输的安全进行。  
&emsp;&emsp;此外，为了能够及时接收到客户端发送过来的请求，服务器端往往要先启动一个守护程序，根据接收到的端口号，启动相应的服务器，然后建立连接，开始通信。  
&emsp;&emsp;TCP协议和UDP协议就是在传输层使用的有代表性的两个协议。TCP协议是一个面向连接的、具有可靠性的数据流型协议，可以给应用程序具备提供可靠性的通信，而UDP协议是不具备可靠性的数据报类型的协议，主要用于同步通信或者强调高速性和实时性的通信中。
### UDP(User Datagram Protocol 用户数据报协议)
&emsp;&emsp;因为UDP比较简单，所以可以进行高速的处理，可以将用户的想法原封不动的传送给服务器，适合：
- 总包数比较少的通信
- 动画和声音的多媒体通信
- 限定在LAN等特定的计算机网络的应用通信
- 需要同步进行的通信
### TCP(Transmission Control Protocol 传输控制协议)
&emsp;&emsp;跟UDP不同，TCP在发送数据的时候对数据进行了控制，通过一系列技术确保包的安全发送，使得有能力实现较为可靠的通信。
- 通过检测是否收到服务器传回来的肯定确认应答来判断数据包是否顺利发送到服务器，在一定时间内如果没有收到则要进行重发
- 重发超时要提前确定，如果多次重发失败，重发超时会成倍的增加，达到一定次数之后若仍失败，则判断发生故障，断开连接
- 在建立和切断连接的过程中，要采取一定的步骤来确保顺利进行，即经典的三次握手和四次挥手
- 通过对数据包进行分段，实现大数据包的稳定快速传输
- 通过窗口控制实现并发传输，提高传输速度
- 在此时，对于服务器端传回的顺序号，发送端会进行判断，如果同样的顺序号连续出现了三次，则认为该段未成功发送到服务器，此时对该段进行重发；少于三次可以认为是服务器接收到了数据但是发送端未收到肯定确认应答
- 服务器对于所有接收到的数据都进行存储，与接收到的顺序无关，只要数据段是连续的，就认为接收完全
...
### 实时通信
&emsp;&emsp;指发送的数据尽可能快的到达通信对方的主机上。TCP由于需要将数据暂时存放在缓冲区里，并不适合实时通信；UDP虽然适合实时通信，但是由于包到达时间的不同，会产生抖动现象。  
&emsp;&emsp;在UDP基础上开发的RTP(Real Time Protocol 实时传输协议)，针对实时通信增加了时间戳和顺序号，用来加强传输的可靠性，再加上RTCP(RTP Control Protocol RTP控制协议)管理通信线路质量。
### UDP报头
- 发送端端口号
- 接收端端口号
- 包的长度
...
## 路由控制协议
### 路由控制
&emsp;&emsp;在实际通信中，在源主机和目标主机之间往往间隔着若干个路由器，要想通过路由器顺利到达目的地，就需要进行路由控制。  
&emsp;&emsp;路由控制分为静态路由控制和动态路由控制。静态路由控制要事先在主机和路由器上进行设定，对于改动比较的不灵活；动态路由控制则是在运行过程中动态的对路由信息进行设定，可以灵活地应对改动或故障，但是相对来讲比较复杂。  
&emsp;&emsp;路由控制协议则分为IGP(Interior Gateway Protocol 内部网关协议)和EGP(Exterior Gateway Protocol 外部网关协议)。
### AS(Autonomous System 自治系统)
&emsp;&emsp;一个区域计算机网络比较大的ISP(Internet Service Provider 因特网服务提供者)，在自治系统内部，使用IGP，在自治系统之间，用EGP。
### 路由控制算法
- 距离向量方法：根据距离方向的不同，来确定目标计算机网络和主机
- 数据链路状态方法：路由器在掌握计算机网络全部连接状态之后再生成路由控制表
### RIP(Routing Information Protocol 路由控制信息协议)
&emsp;&emsp;用于在计算机网络上定期的广播路由控制信息，采用距离向量来确定路由
### RIP2
&emsp;&emsp;在RIP基础上：
- 使用了多播
- 能够处理子网掩码
...
- 增加了外部路由标志
- 增加了认证密钥
### OSPF(Open Shortest Path First 最短路径优先)协议
- 基于数据链路
- 能够处理循环
- 支持子网掩码功能
- 在逻辑上将计算机网络分成区域以减少通信量。
